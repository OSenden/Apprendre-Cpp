...
int main(){
    ...
    else
    {
        ...
        leFichier2 << "+" <<setfill('-') << setw(21) << "+" <<setfill('-') << setw(11) << "+" <<setfill('-') << setw(11) << "+" <<setfill('-') << setw(11) << "+" << endl;
        do{
            ...
            if(leFichier.good())//Si les valeurs ont bien été lues
            {
                ...
                leFichier2 << "| " << left << setw(18) << setfill(' ') << pays ;
                leFichier2 << right << " |" << setw(9) << setfill(' ') << nbOr ;
                leFichier2 << " |" << setw(9) << setfill(' ') << nbArgent ;
                leFichier2 << " |" << setw(9) << setfill(' ') << nbBronze << " |" << endl;
            }
        }while(!leFichier.eof());
